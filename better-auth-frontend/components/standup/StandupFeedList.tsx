...
export function StandupFeedList({
  standups,
  isReacting,
  reactingTarget,
  onToggleReaction,
}: StandupFeedListProps) {
    ...
    <div className="space-y-2.5">
      {standups.map((item) => (
        <motion.article
          ...
          className="rounded-3xl border border-slate-300/60 bg-white/70 p-3.5 shadow-[0_20px_40px_-30px_rgba(15,23,42,0.7)] backdrop-blur-xl"
        >
          <div className="mb-2.5 flex items-center justify-between border-b border-slate-200/80 pb-1.5">
            <h3 className="text-sm font-semibold text-slate-900">{item.user.name}</h3>
            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500">{formatTime(item.createdAt)}</span>
          </div>

          <div className="grid gap-1.5 text-sm text-slate-700">
            <p className="rounded-lg bg-slate-50/80 px-3 py-1.5 leading-relaxed">
          ...
          <div className="mt-2.5 flex flex-wrap items-center gap-2">
            {EMOJI_CHOICES.map((emoji) => {
              ...
              const isLoadingReaction =
                isReacting && reactingTarget?.standupId === item.id && reactingTarget.emoji === emoji;
              return (
                <button
                  key={emoji}
                  type="button"
                  ...
                  className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200 ${
                    reactedByMe
